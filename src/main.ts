  ...
  <div style="display: flex; height: 100vh; font-family: sans-serif; background: #1a1a1a; color: white; overflow: hidden;">
    <div style="width: 260px; background: #2c3e50; padding: 20px; display: flex; flex-direction: column; gap: 15px; z-index: 10;">
      ...
      <div style="display: flex; flex-direction: column; gap: 8px;">
        ...
        <button id="join-room-btn" style="background: #3498db; color: white; border: none; padding: 12px; border-radius: 8px; cursor: pointer; font-weight: bold;">会議に参加</button>
      </div>
      <div id="status-area" style="font-size: 11px; padding: 10px; border-radius: 5px; background: rgba(0,0,0,0.3); border-left: 4px solid #95a5a6; min-height: 60px;">待機中</div>
      <div style="margin-top: auto;">
        <button id="hangup-btn" style="background: #e74c3c; color: white; border: none; padding: 10px; border-radius: 8px; cursor: pointer; width: 100%;">リセット</button>
      </div>
    </div>
...
function join() {
  ...
  if (!room || !pass) return alert("ルーム名とパスワードを入れてね");
  ...
  peer.on('error', (err) => {
    // ★ここが大事！「相手がいない」だけのエラーなら画面に表示しない
    if (err.type === 'peer-unavailable') {
      console.log("空席をスキップしました");
      return; 
    }
...
function handleCall(call: any) {
  call.on('stream', (stream: MediaStream) => {
    ...
    v.style.cssText = "width: 100%; height: 100%; border-radius: 8px; width:100%; height:100%; object-fit:cover;";
