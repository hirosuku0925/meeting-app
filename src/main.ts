...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <div style="display: flex; height: 100vh; font-family: sans-serif; overflow: hidden; background: #f0f2f5;">
    <div style="flex: 1; display: flex; flex-direction: column; align-items: center; padding: 20px; overflow-y: auto;">
      <h1 style="color: #333; margin-bottom: 20px;">キツネ会議室 (VRM対応版)</h1>
      ...
      <div class="card" style="width: 100%; max-width: 500px; margin-top: 20px; padding: 20px; border-radius: 16px; background: #fff; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
        <div style="background: #f8f9fa; padding: 15px; border-radius: 12px; margin-bottom: 15px;">
           <label style="font-size: 11px; font-weight: bold; color: #1976D2;">🏞 背景画像を変更</label>
           <input type="file" id="bg-upload" accept="image/*" style="width: 100%; font-size: 10px; margin-top: 5px;">
        </div>
        <div style="display: flex; gap: 8px; justify-content: center; margin-bottom: 15px;">
          <button id="avatar-mode-btn" style="background-color: #646cff; color: white; padding: 10px 15px; border-radius: 8px; border:none; cursor: pointer;">👤 アバター: ON</button>
...
let isAvatarMode = true; // デフォルトでON
...
function animateVrm(faceLandmarks: any) {
  if (!currentVrm || !faceLandmarks || !isAvatarMode) return;
...
faceMesh.onResults((res) => {
  if (res.multiFaceLandmarks?.[0]) animateVrm(res.multiFaceLandmarks[0]);
  // 背景やアバターモードに応じた描画
  if (currentVrm) {
