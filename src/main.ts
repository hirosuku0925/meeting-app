  ...
  <div style="display: flex; height: 100vh; font-family: sans-serif; background: #1a1a1a; color: white; overflow: hidden;">
    <div style="width: 260px; background: #2c3e50; padding: 20px; display: flex; flex-direction: column; gap: 15px; z-index: 10; box-shadow: 2px 0 10px rgba(0,0,0,0.5);">
      ...
      <div style="display: flex; flex-direction: column; gap: 8px;">
        ...
        <input id="room-id-input" type="text" placeholder="例: team-a" style="padding: 10px; border-radius: 5px; border: none; color: #333;">
        ...
        <button id="join-room-btn" style="background: #3498db; color: white; border: none; padding: 12px; border-radius: 8px; cursor: pointer; font-weight: bold; margin-top: 5px;">部屋に入室する</button>
      </div>
    ...
    <div style="flex: 1; display: flex; flex-direction: column; background: #000; position: relative;">
      <div id="main-display" style="flex: 1; display: flex; align-items: center; justify-content: center; padding: 20px;">
        <video id="big-video" autoplay playsinline style="max-width: 100%; max-height: 100%; border-radius: 12px; box-shadow: 0 0 30px rgba(0,0,0,0.8);"></video>
      </div>
      ...
      <div id="video-grid" style="height: 180px; background: rgba(0,0,0,0.5); display: flex; gap: 15px; padding: 15px; overflow-x: auto; border-top: 1px solid #333;">
        <div style="position: relative; min-width: 220px; height: 100%;">
          <video id="local-video" autoplay playsinline muted style="width: 100%; height: 100%; object-fit: cover; border-radius: 10px; border: 3px solid #646cff; background: #222;"></video>
...
async function init() {
  try {
    ...
    bigVideo.srcObject = localStream; // 最初は自分をメインに
  } catch (e) {
...
document.querySelector('#hangup-btn')?.addEventListener('click', () => {
  if (confirm("退出してリロードしますか？")) location.reload();
});
